   ...
   const handleEdit = (itemId) => {
    ...
    if (inputValue !== null) {
      const updatedEntries = entries.map((entry) =>
        entry.id === itemId ? { ...entry, value: parseFloat(inputValue) } : entry
      );
            ...
            <li key={item.id} className="py-2.5">
              <div className="group flex justify-between gap-2 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 hidden cursor-pointer font-medium text-red-500 group-hover:inline-block"  onClick={() => handleDelete(item.id)}>
                    Delete
                  </span>
                  <span
